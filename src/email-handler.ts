...
import { getMailbox, saveEmail, saveAttachment, saveTempEmail } from './database';
  ...
  try {
    ...
    if (!mailbox) {
      ...
      if (email.text || email.html) {
        await saveTempEmail(env.DB, email.to[0].address, email.from.address, email.text || null, email.html || null);
        console.log('已保存为临时邮件');
      }
      return;
    }
